<template>
  <div class="list">
    <article v-for="(pokemon, index) in pokemonsFiltered" v-bind:key="index" v-on:click="showPokemonDetail(pokemon.url)">
      <h3>{{pokemon.name}}</h3>
...
export default {
  ...
  methods: {
    showPokemonDetail(url){
      this.$emit('event_from_child',url)
    },
